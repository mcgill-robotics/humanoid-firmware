...
int XL320::getJointPosition(int id) { return getJointPosition(id, NULL); }
...
int XL320::sendPacket(int id, int Address, int value) {
  ...
  */
  ...
  Packet p(txbuffer, bufsize, id, 0x03, 4, DXL_LOBYTE(Address),
           DXL_HIBYTE(Address), DXL_LOBYTE(value), DXL_HIBYTE(value));
...
int XL320::flush() { this->stream->flush(); }
...
int XL320::RXsendPacket(int id, int Address, int size) {
  ...
  */
  ...
  Packet p(txbuffer, bufsize, id, 0x02, 4, DXL_LOBYTE(Address),
           DXL_HIBYTE(Address), DXL_LOBYTE(size), DXL_HIBYTE(size));
...
XL320::Packet::Packet(unsigned char *data, size_t data_size, unsigned char id,
...
unsigned char XL320::Packet::getId() { return data[4]; }

int XL320::Packet::getLength() { return data[5] + ((data[6] & 0xff) << 8); }

int XL320::Packet::getSize() { return getLength() + 7; }

int XL320::Packet::getParameterCount() { return getLength() - 3; }

unsigned char XL320::Packet::getInstruction() { return data[7]; }

unsigned char XL320::Packet::getParameter(int n) { return data[8 + n]; }
